...
use clap::{App, Arg};
use vmm::{KernelConfig, MemoryConfig, VMMConfig, VcpuConfig, BlockConfig, NetConfig};
...
impl CLI {
    ...
    pub fn launch(cmdline_args: Vec<&str>) -> result::Result<VMMConfig, String> {
        ...
        Ok(VMMConfig {
            ...
            block_config:
                match matches.value_of("block") {
                    Some(value) => {
                        Some(BlockConfig::try_from(value.to_string()).map_err(|e| {
                            eprintln!("{}", help_msg);
                            format!("{}", e)
                        })?)
                    },
            ...
            network_config:
            match matches.value_of("net") {
                Some(value) => {
                    Some(NetConfig::try_from(value.to_string()).map_err(|e| {
                        eprintln!("{}", help_msg);
                        format!("{}", e)
                    })?)
                },
